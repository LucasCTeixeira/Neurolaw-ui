...
function pesquisar(form){
	location.href='${pageContext.servletConfig.servletContext.contextPath}/pagamentos/pesquisar?inicio='+form.inicio.value+'&termino='+form.termino.value+'&filial='+form.filial.value+'&produto='+form.produto.value+'&fornecedor='+form.fornecedor.value;
} 
</script>
			...
			<tbody>
				<c:forEach items="${registros}" var="registro">
					<tr>
							...
							<c:choose>
							  <c:when test="${registro.entrada.key ne null}">
								<table class="display" style="width: 100%" align="left">
									...
									<c:forEach items="${registro.entrada.itens}" var="item">
										<tr style="border-bottom-width: 1px; border-bottom-style: solid; border-bottom-color: gray">
											<td><c:out value="${item.indice}"/></td>
											<td><c:out value="${item.produto.descricao}"/></td>
											<td><c:out value="${item.fornecedor.nome}"/></td>
						...
						<td><fmt:formatNumber type="currency" currencySymbol="R$" value="${registro.valorDeclarado-registro.valorPago}"/></td>
					</tr>
					<c:set var = "totalDeclarado" value = "${totalDeclarado + registro.valorDeclarado}"/>
					<c:set var = "totalPago" value = "${totalPago + registro.valorPago}"/>
				</c:forEach>
			</tbody>
			<tfoot align="left">
				<tr>
				...
				<tr>
					...
					<td><fmt:formatNumber type="currency" currencySymbol="R$" value="${totalPago}" minFractionDigits="2" maxFractionDigits="2"/></td>
					<th>&nbsp;</th>
					<td><fmt:formatNumber type="currency" currencySymbol="R$" value="${totalDeclarado-totalPago}" minFractionDigits="2" maxFractionDigits="2"/></td>
				</tr>
			</tfoot>
		</table>
	</div>
